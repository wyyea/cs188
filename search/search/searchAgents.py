...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        for corner in self.corners:
            if corner == self.startingPosition:
                reachedCorners = (corner,) + reachedCorners
                break
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [
            ...
            reachedCorners = state[1]
            for corner in self.corners:
                if corner == (nextx, nexty) and (corner not in reachedCorners):
                    reachedCorners = (corner,) + reachedCorners
                    break
            successors.append((((nextx, nexty), reachedCorners), action, 1))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    if problem.isGoalState(state):
        return 0
    notReached = list(set(problem.corners) - set(state[1]))
    minheu = None
    for order in itertools.permutations(notReached):
        currentPos = state[0]
        heu = 0
        for pos in order:
            heu += util.manhattanDistance(currentPos, pos)
            currentPos = pos
        minheu = heu if minheu is None else min(minheu, heu)
    return minheu
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    "*** YOUR CODE HERE ***"
    notReached = foodGrid.asList()
    len = notReached.__len__()
    if problem.path is None:
        problem.path = 1 if len > 9 else 2
    # path 1 is not consistent, is there any good consistent heuristic for large num of food
    if problem.path == 1:
        heu = 0
        for _ in range(len):
            dis = [util.manhattanDistance(position, food) for food in notReached]
            mindis = min(dis)
            idx = dis.index(mindis)
            heu += mindis
            position = notReached.pop(idx)
        return heu
    minheu = None
    for order in itertools.permutations(notReached):
        curPos = position
        heu = 0
        for food in order:
            heu += util.manhattanDistance(food, curPos)
            curPos = food
        minheu = heu if minheu is None else min(heu, minheu)
    return minheu
